...
1. Settings → Environments: создать `dev`, `stage`, `prod` (+ `dev-plan`, `stage-plan`,
   `prod-plan` для стадии планирования).
2. В каждом окружении добавить секреты `YC_SA_KEY_JSON`, `TFSTATE_ACCESS_KEY`,
   `TFSTATE_SECRET_KEY` и переменную `SSH_PUBLIC_KEY`.
3. Для `stage` и `prod` включить Required reviewers — это и есть кнопка approval:
   job не стартует, пока подтверждение не получено. Для `prod` дополнительно
   ограничить Deployment branches значением `main`.
